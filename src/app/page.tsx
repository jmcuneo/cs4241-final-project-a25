...
export default function StartPage() {
    ...
    return (
        <main
            className="min-h-dvh flex items-center justify-center p-6 bg-gradient-to-b from-zinc-900 to-zinc-800 text-zinc-100">
            <div className="w-full max-w-3xl rounded-2xl p-6 shadow-xl bg-zinc-900/60 backdrop-blur">
                ...
                <section className="mt-6 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-xl border border-zinc-700 p-4">
                        {status === "loading" ? (
                            ...
                            <div>
                                <h2 className="font-semibold mb-2">Hello, {session.user.name || session.user.email}</h2>
                                {session.user.image && (
                                    <img
                                        src={session.user.image}
                                        alt="Profile"
                                        className="w-12 h-12 rounded-full border-2 border-zinc-700 mb-2"
                                    />
                                )}
                                <div className="text-sm mt-2 flex gap-4 text-zinc-400">
                                    <span>Wins: 0</span>
                                    <span>Losses: 0</span>
                                </div>
                                <button
                    ...
                    <div className="rounded-xl border border-zinc-700 p-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <label
                                className={`cursor-pointer select-none rounded-xl border p-3 ${deck === "nordic" ? "border-sky-400 shadow-[0_0_0_2px] shadow-sky-400/30" : "border-zinc-700 hover:border-zinc-600"}`}>
                                <input
                                    type="radio"
                                    name="deck"
                                    value="nordic"
                            ...
                            <label
                                className={`cursor-pointer select-none rounded-xl border p-3 ${deck === "egyptian" ? "border-amber-400 shadow-[0_0_0_2px] shadow-amber-400/30" : "border-zinc-700 hover:border-zinc-600"}`}>
                                <input
                                    type="radio"
                                    name="deck"
                                    value="egyptian"
                                ...
                                bg-gradient-to-br from-amber-900 to-yellow-700">
                                    ...
                                    <div
                                        className="absolute inset-0 opacity-25 [background:repeating-linear-gradient(135deg,black,black_6px,transparent_6px,transparent_12px)] mix-blend-soft-light"/>
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                    <button
                        onClick={handlePlay}
                        className="h-12 mt-6 align-bottom rounded-lg bg-sky-500 px-4 py-2 font-semibold hover:bg-sky-600 active:translate-y-px"
                    >
                        Play
                    </button>
                    <button
                        onClick={() => setShowRules(true)}
                        className="h-12 mt-6 rounded-lg border border-zinc-600 px-4 py-2 hover:bg-zinc-800"
                    >
                        Rules
                    </button>
                    ...
                    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
                        <div className="w-full max-w-xl rounded-2xl bg-zinc-900 p-6 shadow-xl">
                            ...
                            <ol className="mt-4 list-decimal space-y-2 pl-5 text-sm text-zinc-300">
                                ...
                                <li>On your turn, flip your top card to the table. Higher rank wins the round (A
                                    high).
                                </li>
                                <li>On a tie, it&#39;s &#34;WAR&#34;: both players burn 3 cards face-down, then flip a 4th; winner
                                    takes all flipped cards.
                                </li>
