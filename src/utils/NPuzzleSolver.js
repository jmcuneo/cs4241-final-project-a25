...
export function isSolvable(arr, size = 4) {
    // Count inversions
    let inversions = 0;
    const flatArray = arr.filter(x => x !== 0); // Remove empty space
    ...
    if (size === 4) {
        ...
        // If empty space is on even row from bottom, then number of inversions should be odd
        // If empty space is on odd row from bottom, then number of inversions should be even
        ...
        // For odd-sized grids, solvable if inversions are even
        return inversions % 2 === 0;
    }
}

// Generate a solvable shuffled puzzle
export function generateSolvablePuzzle(size = 4) {
    let arr;
    do {
        arr = [...Array(size * size - 1).keys()].map(i => i + 1);
        arr.push(0); // Add empty space
...
// Utility functions to convert between 1D array and 2D grid formats
export function arrayTo2D(arr, size = 4) {
    const grid = [];
    for (let i = 0; i < size; i++) {
        grid[i] = [];
        for (let j = 0; j < size; j++) {
            const value = arr[i * size + j];
            // Convert 0 (empty) to "" for solver compatibility
            grid[i][j] = value === 0 ? "" : value;
        }
    }
    return grid;
}

export function gridTo1D(grid) {
    const arr = [];
    for (let i = 0; i < grid.length; i++) {
        for (let j = 0; j < grid[i].length; j++) {
            // Convert "" (empty) back to 0 for React component
            arr.push(grid[i][j] === "" ? 0 : grid[i][j]);
        }
    }
    return arr;
}

export function solvePuzzle(tiles) {
    try {
        console.log('Starting solve for tiles:', tiles);
        const grid = arrayTo2D(tiles);
        console.log('Converted to 2D grid:', grid);
        ...
        if (!solution || solution.length === 0) {
            console.log('Solver returned null or empty solution');
            return null;
        }
        
        // Convert solution steps to 1D array moves
        const moves = solution.map(step => {
            const size = grid.length;
            const pieceIndex = step.piece.y * size + step.piece.x;
            const emptyIndex = step.empty.y * size + step.empty.x;
            return { pieceIndex, emptyIndex, number: step.number };
        });
        
        console.log('Converted moves:', moves.slice(0, 5)); // Log first 5 moves
        return moves;
    } catch (error) {
